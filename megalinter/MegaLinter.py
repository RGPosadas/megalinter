...
class Megalinter:
    ...
    def list_files_git_diff(self):
        ...
        if "origin/HEAD" not in [ref.name for ref in repo.refs]:
            # Try to fetch HEAD from origin, because it isn't cached locally.
            repo.git.fetch("origin", "HEAD:refs/remotes/origin/HEAD")
        diff = repo.git.diff("origin/HEAD", name_only=True)
